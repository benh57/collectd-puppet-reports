...
def compute_metrics(data):
    ...
    h.update(
        tridict('changes', safe_get(data, ['metrics', 'changes', 'values'], {})))
    h.update(
        tridict('events', safe_get(data, ['metrics', 'events', 'values'], {})))
    h.update(
        tridict('resources', safe_get(data, ['metrics', 'resources', 'values'], {})))
    h.update(
        tridict('time', safe_get(data, ['metrics', 'time', 'values'], {})))
    return h
...
def map_value(node):
    if isinstance(node, yaml.nodes.MappingNode):
        dicts = map(
            lambda x: dict({map_value(x[0]): map_value(x[1])}), node.value)
    ...
    with open(last_report_file, "r") as stream:
        ...
        for k in results:
            ...
            try:
                ...
                logger('warn', ("value %s => %s for %s cannot be parsed to float" % (
                    k, results[k], last_report_file)))
                val.values = [0.0]
            val.dispatch()
